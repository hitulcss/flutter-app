...
class BatchQuizScreen extends StatefulWidget {
  final String batchId;
  final bool isFullScreen;
  final bool isLocked;
  const BatchQuizScreen({super.key, required this.batchId, required this.isFullScreen, required this.isLocked});

  @override
  State<BatchQuizScreen> createState() => _BatchQuizScreenState();
}

class _BatchQuizScreenState extends State<BatchQuizScreen> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: FutureBuilder<ListOfQuizModel>(
          future: RemoteDataSourceImpl().getListOfQuizByBatchid(id: widget.batchId),
          builder: (context, snapshot) {
            if (ConnectionState.done == snapshot.connectionState) {
              if (snapshot.hasData) {
                analytics.logScreenView(screenName: "app_datch_quiz", screenClass: "QuizListScreen");
                ...
                if (snapshot.data!.status!) {
                  return snapshot.data!.data!.notAttempted!.isEmpty && snapshot.data!.data!.isAttempted!.isEmpty
                      ? EmptyWidget(image: SvgImages.emptyquiz, text: Preferences.appString.noquiz ?? "No Quiz found")
                      : SingleChildScrollView(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              snapshot.data!.data!.notAttempted!.isNotEmpty
                                  ? quizbody(quizdata: snapshot.data!.data!, isAttempted: false)
                                  : Center(
                                      child: Container(
                                        ...
                                        child: EmptyWidget(image: SvgImages.emptyquiz, text: Preferences.appString.noTests ?? "No Test found"),
                                      ),
                                    ),
                              snapshot.data!.data!.isAttempted!.isNotEmpty
                                  ...
                                  : Container(),
                              snapshot.data!.data!.isAttempted!.isNotEmpty ? quizbody(quizdata: snapshot.data!.data!, isAttempted: true) : Container(),
                            ],
                          ),
                        );
                } else {
  ...
  Widget quizbody({required ListOfQuizModelData quizdata, required bool isAttempted}) {
    return Padding(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          return quizcard(context, isAttempted ? quizdata.isAttempted![index] : quizdata.notAttempted![index], isAttempted, index);
        },
      ),
    );
  }

  Widget quizcard(BuildContext context, quizModelData, bool isAttempted, int index) {
    return Container(
      height: 130,
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Text(
                                        //'Continue'
                                        isAttempted ? Languages.result : Preferences.appString.sContinue ?? Languages.continueText,
                          ...
                          ? Row(
                              ...
                              children: [
                                ...
                                RounderTextwidget(
                                  ...
                                  centertextcolor: ColorResources.textWhite,
                                  footer: Preferences.appString.correct ?? 'Correct',
                                ),
                                RounderTextwidget(
                                  backgroundcolor: const Color(0XFFFB5259),
                                  center: response.data!.summary!.wrongAnswers.toString(),
                                  centertextcolor: ColorResources.textWhite,
                                  footer: Preferences.appString.incorrect ?? 'Incorrect',
                                ),
                                RounderTextwidget(
                                  ...
                                  footer: Preferences.appString.skipped ?? Languages.skipped,
                                ),
                              ],
                            )
                          : Row(
                              ...
                              children: [
                                RounderTextwidget(
                                  ...
                                  center: response.data!.summary!.noOfQues.toString(),
                                  centertextcolor: ColorResources.textWhite,
                                  footer: Preferences.appString.total ?? Languages.total,
                                ),
                                RounderTextwidget(
                                  backgroundcolor: const Color(0XFFFFB800),
                                  center: response.data!.summary!.attempted.toString(),
                                  centertextcolor: ColorResources.textWhite,
                                  footer: Preferences.appString.attempted ?? Languages.attempted,
                                ),
                                RounderTextwidget(
                                  backgroundcolor: const Color(0XFFBBE3FF),
                                  center: response.data!.summary!.skipped.toString(),
